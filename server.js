...
const inventoryLogSchema = new mongoose.Schema(
  {
    ...
    action: {
      type: String,
      enum: ["purchase", "restock", "adjustment"],
      required: true,
    },
...
app.post("/api/sweets", authenticateToken, authorizeAdmin, async (req, res) => {
  try {
    ...
    if (existingSweet)
      return res.status(409).json({ success: false, message: "Sweet exists" });
    ...
    res
      .status(201)
      .json({ success: true, message: "Sweet added", data: newSweet });
  } catch (error) {
    res
      .status(500)
...
app.get("/api/sweets", authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: { sweets, pagination: { total, page, limit } },
    });
  } catch (error) {
    res
      .status(500)
  ...
  async (req, res) => {
    try {
      ...
      if (updates.quantity !== undefined)
        sweet.isAvailable = updates.quantity > 0;
  ...
  try {
    ...
    if (!sweet.isAvailable || sweet.quantity < quantity) {
      return res.status(400).json({
        success: false,
        message: `Insufficient stock: ${sweet.quantity}`,
      });
    }
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: "Restocked",
        data: { sweet: sweet.name, newTotal: sweet.quantity },
      });
    } catch (error) {
      res.status(500).json({ success: false, error: error.message });
    }
...
// User History
// User History - Full List with User Details
app.get("/api/purchases/my-history", authenticateToken, async (req, res) => {
  try {
    // 🟢 Removed skip and limit to show the full history for grouping
    const purchases = await Purchase.find({ user: req.user._id })
      .populate("sweet", "name category price")
      .populate("user", "username") // 🟢 Added to fix the "Empty Username" issue
      .sort({ purchaseDate: -1 });
  ...
  async (req, res) => {
    try {
      ...
      const topSweets = await Purchase.aggregate([
        ...
        {
          $project: {
            name: "$sweetDetails.name",
            category: "$sweetDetails.category",
            totalSold: 1,
            totalRevenue: 1,
          },
        },
      ...
      res.json({
        ...
        data: {
          overview: {
            ...
            totalItemsSold: revenue.totalItems,
          },
